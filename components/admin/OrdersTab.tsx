...
const OrdersTab: React.FC<OrdersTabProps> = ({ resellers, setResellers }) => {
    ...
    const updateOrderStatus = (resellerId: string, orderId: string, newStatus: ResellerOrder['status']) => {
        const updatedResellers = resellers.map(r => {
            if (r.id === resellerId) {
                ...
                // 2. Crear mensaje de notificación automática
                ...
                // 3. Si se entrega, actualizar stock del revendedor
                let updatedStock = r.stock;
                if (newStatus === 'Entregado') {
                   const order = r.orders.find(o => o.id === orderId);
                   if (order) {
                       // Clonar stock actual
                       updatedStock = r.stock.map(p => ({...p})); 
                       order.items.forEach(item => {
                           const prodIndex = updatedStock.findIndex(p => p.id === item.id);
                           if (prodIndex >= 0) {
                               updatedStock[prodIndex].stock += item.quantity;
                           } else {
                               // Si es un producto nuevo que no tenia en stock
                               // (Esto requiere manejo cuidadoso si el tipo Product no coincide exactamente con CartItem)
                               // Por simplicidad, asumimos que ya existe en el array inicializado con 0
                           }
                       });
                   }
                }
    ...
    // Render logic remains mostly the same, ensuring 'Pendiente' maps to 'Preparando' visually if desired
    // ... (El resto del renderizado sigue igual que el archivo anterior que te pasé, solo cambia la lógica de updateOrderStatus)
    
    // Aplanar y ordenar pedidos
